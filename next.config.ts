...
export default function DriverCard({ driver }: { driver: DriverProfile & { profiles?: any } }) {
  ...
  return (
    <div style={{
      background: '#fafaf8', borderRadius: 16, border: '1px solid #ececea',
      padding: 24, transition: 'transform 0.2s',
    }}
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 16 }}>
        <div style={{
          width: 52, height: 52, borderRadius: '50%', background: '#d4edd8',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 18, fontWeight: 700, color: '#1a5c28', fontFamily: 'sans-serif', flexShrink: 0,
        }}>
          {initials}
        </div>
        <div>
      ...
      <div style={{ fontSize: 13, fontFamily: 'sans-serif', color: '#555', lineHeight: 2, marginBottom: 12 }}>
        ...
        {driver.license_verified && <div>✅ <strong>License:</strong> Verified</div>}
      </div>

      {/* Platforms */}
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14, alignItems: 'center' }}>
        <span style={{ fontSize: 12, color: '#888', fontFamily: 'sans-serif' }}>Platforms:</span>
        {driver.platforms?.map(p => <PlatformBadge key={p} name={p} />)}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
        <Link href={`/drivers/${driver.id}`} style={{
          flex: 1, background: 'none', border: '1px solid #1a2e1a', color: '#1a2e1a',
          borderRadius: 8, padding: '9px', fontSize: 13, textDecoration: 'none',
          fontFamily: 'sans-serif', textAlign: 'center', display: 'block',
        }}>
          View Profile
        </Link>
      </div>
