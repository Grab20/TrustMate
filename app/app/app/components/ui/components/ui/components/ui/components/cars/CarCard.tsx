...
export default function CarCard({ car }: { car: any }) {
  return (
    <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #ececea', overflow: 'hidden', transition: 'transform 0.2s' }}
      ...
      <div style={{ padding: '16px 20px 20px' }}>
        <h3 style={{ margin: '0 0 2px', fontSize: 18, fontWeight: 700, color: '#0d1f0e' }}>{car.make} {car.model}</h3>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
          ...
          <Link href={`/cars/${car.id}`} style={{ background: '#1a2e1a', color: '#fff', borderRadius: 8, padding: '9px 18px', fontSize: 13, textDecoration: 'none', fontFamily: 'sans-serif' }}>
            View & Apply
          </Link>
        </div>
      </div>
